...
void default_layer_set_keymap(uint8_t code) {
    switch (code) {
        ...
        case KC_D:
            ...
            break;
        case KC_G:
            default_layer_set(1UL << _GAME_DF);
            break;
        case KC_H:
            default_layer_set(1UL << _GAME_ALT);
            break;
        default:
            break;
    }
...
// Different from Miryoku: RHS thumb keys can be : {Bksp, Ent, Del} instead of {Ent, Bksp, Del}
